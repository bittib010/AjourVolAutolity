...
run_os_commands() {
    ...
    if [[ "$os_type" == "win" ]]; then
        ...
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.info > $unique_output_directory/info.csv"
        "strings -a -td $dump > $unique_output_directory/strings.txt && strings -a -td -el $dump  > $unique_output_directory/strings.txt && vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.strings.Strings --strings-file $unique_output_directory/strings.txt > $unique_output_directory/strings.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv timeliner > $unique_output_directory/timeliner.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.bigpools > $unique_output_directory/bigpools.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.cachedump > $unique_output_directory/cachedump.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.callbacks.Callbacks > $unique_output_directory/callbacks.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.cmdline.CmdLine > $unique_output_directory/cmdline.csv &"
        #"vol -f {crashdump} --log $unique_output_directory/Volatility.log -q -r csv windows.crashinfo.Crashinfo > $unique_output_directory/crashinfo.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.devicetree.DeviceTree > $unique_output_directory/devicetree.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.dlllist.DllList > $unique_output_directory/dlllist.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.driverirp.DriverIrp > $unique_output_directory/driverirp.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.drivermodule.DriverModule > $unique_output_directory/drivermodule.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.driverscan.DriverScan > $unique_output_directory/driverscan.csv &"

        "mkdir $unique_output_directory/dumpfiles"
        "mkdir $unique_output_directory/clamscan_tmp"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q  -o $unique_output_directory/dumpfiles windows.dumpfiles.DumpFiles && clamscan $unique_output_directory/dumpfiles/* > $unique_output_directory/clamscan_tmp/clamscan.txt && convert_clamscan_output_to_csv $unique_output_directory/clamscan_tmp/clamscan.txt $unique_output_directory/clamscan.csv &"
        # add a "file" check and make it csv
        
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.dumpfiles.DumpFiles > $unique_output_directory/dumpfiles.csv &"        
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.envars.Envars > $unique_output_directory/envars.csv &"        
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.filescan.FileScan > $unique_output_directory/filescan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.getservicesids.GetServiceSIDs > $unique_output_directory/getservicesids.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.getsids.GetSIDs > $unique_output_directory/getsids.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.handles.Handles > $unique_output_directory/handles.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.hashdump.Hashdump > $unique_output_directory/hashdump.csv&"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.joblinks.JobLinks > $unique_output_directory/joblinks.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.ldrmodules.LdrModules > $unique_output_directory/ldrmodules.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.lsadump.Lsadump > $unique_output_directory/lsadump.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.malfind.Malfind > $unique_output_directory/malfind.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.mbrscan.MBRScan > $unique_output_directory/mbrscan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.memmap.Memmap > $unique_output_directory/memmap.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.mftscan.MFTScan > $unique_output_directory/mftscan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.modscan.ModScan > $unique_output_directory/modscan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.modules.Modules > $unique_output_directory/modules.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.mutantscan.MutantScan > $unique_output_directory/mutantscan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.netscan.NetScan > $unique_output_directory/netscan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.netstat.NetStat > $unique_output_directory/netstat.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.poolscanner.PoolScanner > $unique_output_directory/poolscanner.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.privileges.Privs > $unique_output_directory/privileges.csv &"

        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.pslist.PsList > $unique_output_directory/pslist.csv &"

        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.psscan.PsScan > $unique_output_directory/psscan.csv && head -n 1 \"$unique_output_directory/psscan.csv\" > \"$unique_output_directory/taskhost_triage.csv\" && grep -E -i \"taskhost\" $unique_output_directory/pslist.csv >> $unique_output_directory/taskhost_triage.csv &"

        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.registry.certificates.Certificates > $unique_output_directory/registry.csv &"
        "mkdir $unique_output_directory/regdumps/"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv -o $unique_output_directory/regdumps/ windows.registry.hivelist.HiveList --dump &"

        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.registry.hivelist.HiveList > $unique_output_directory/registry_hivelist.csv && printkeys_for_each_hive $unique_output_directory/registry_hivelist.csv $dump $unique_output_directory"
        
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.registry.printkey.PrintKey > $unique_output_directory/registry_printkey.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.registry.userassist.UserAssist > $unique_output_directory/registry_userassist.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.sessions.Sessions > $unique_output_directory/sessions.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.skeleton_key_check.Skeleton_Key_Check > $unique_output_directory/skeleton_key_check.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.ssdt.SSDT > $unique_output_directory/ssdt.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.statistics.Statistics > $unique_output_directory/Statistics.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.svcscan.SvcScan > $unique_output_directory/svcscan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.symlinkscan.SymlinkScan > $unique_output_directory/symlinkscan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.vadinfo.VadInfo > $unique_output_directory/vadinfo.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.vadwalk.VadWalk > $unique_output_directory/vadwalk.csv &"

        # Yara
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.vadyarascan.VadYaraScan --yara-rules "https://raw.githubusercontent.com/reversinglabs/reversinglabs-yara-rules/develop/yara/backdoor/Win64.Backdoor.Konni.yara" > $unique_output_directory/vadyarascan.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv yarascan.YaraScan --yara-rules "https://raw.githubusercontent.com/reversinglabs/reversinglabs-yara-rules/develop/yara/backdoor/Win64.Backdoor.Konni.yara" > $unique_output_directory/yarascan.csv &"

        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.verinfo.VerInfo > $unique_output_directory/verinfo.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.virtmap.VirtMap > $unique_output_directory/virtmap.csv &"
        "vol -f $dump --log $unique_output_directory/Volatility.log -q -r csv windows.iat.IAT > $unique_output_directory/iat.csv &"

        # PLUGINS
        "vol -f $dump --log  $unique_output_directory/Volatility.log -q -r csv windows.cobaltstrike.CobaltStrike > $unique_output_directory/cobaltstrike.csv &"
...
printkeys_for_each_hive() {
    ...
    do
        ...
        vol -f "$dump" --log "$outpath/Volatility.log" -q -r csv windows.registry.printkey --offset "$current_offset" --recurse >> "$outpath/printkey_${filename_key}.csv"
